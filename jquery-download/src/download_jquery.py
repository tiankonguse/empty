...
def save(name, title, description, keywords, content):
    filePath = ("../data/jquery/%s" % (name))
    try:
        f = open(filePath, 'w')
        f.write("---\n")
        f.write("layout: jquery\n")
        f.write("title: %s\n"%(title))
...
def downjQuery(link, pre):
    ...
    #description
    description = soup.findAll("meta",{"name" : "description"})[0].get("content")
    description = pre + description
    
    
    #keywords
    keywords = soup.findAll("meta",{"name" : "keywords"})[0].get("content")
...
if __name__ == "__main__":
    if not lockFile("jquery.lock.pid"):  
        sys.exit(0) 
    main ( )
