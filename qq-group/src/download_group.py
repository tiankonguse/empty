...
def runQQ(ret):
    ...
    def readInfo ( info, ret):
        try:
            ...
            #conn = MySQLdb.connect ( host=dbcfg["host"], user=dbcfg["user"], passwd=dbcfg["passwd"], db=dbcfg["db"], port=dbcfg["port"], charset='utf8')
            cur = conn.cursor ( )
            sql = "SELECT c_qq FROM t_qq_info where c_state = 0 order by c_qq desc limit 1;"
            cur.execute ( sql)
            results = cur.fetchall ( )
...
def runGroup(ret):
    ...
    def readGroupInfo ( info, ret):
        try:
            ...
            #conn = MySQLdb.connect ( host=dbcfg["host"], user=dbcfg["user"], passwd=dbcfg["passwd"], db=dbcfg["db"], port=dbcfg["port"], charset='utf8')
            cur = conn.cursor ( )
            sql = "SELECT c_group FROM d_qq.t_qq_group_info where c_state = 0 limit 1;"
            cur.execute ( sql)
            results = cur.fetchall ( )
...
if __name__ == "__main__":
    if not lockFile("group.lock.pid"):  
        sys.exit(0) 
    main ( )
